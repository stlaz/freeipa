...
def configure_ldap_conf(
    ...
    opts = [
        ...
        ldapconf.setOption('nss_base_passwd', str(
                DN(('cn', 'users'), ('cn', 'accounts'), cli_basedn)
            ) + '?sub'),
        ldapconf.setOption('nss_base_group', str(
...
def configure_krb5_conf(
    ...
    opts = [
        ...
        krbconf.emptyLine(),
        {
            'name': 'includedir',
            'type': 'option',
            'value': paths.COMMON_KRB5_CONF_DIR, 'delim': ' '
        }
    ]
    ...
    # [libdefaults]
    libopts = [krbconf.setOption('default_realm', cli_realm)]
    ...
    opts.extend([{'name': 'libdefaults', 'type': 'section', 'value': libopts},
                 krbconf.emptyLine()])
    ...
    # [domain_realm]
    dropts = [krbconf.setOption('.' + cli_domain, cli_realm),
    ...
    if cli_domain != client_domain:
        dropts.extend([krbconf.setOption('.'+client_domain, cli_realm),
                       krbconf.setOption(client_domain, cli_realm)])

    opts.extend([krbconf.setSection('domain_realm', dropts),
                 krbconf.emptyLine()])
